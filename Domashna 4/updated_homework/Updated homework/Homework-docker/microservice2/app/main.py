...
class Filter2(Filter):
    ...
    def read_issuers_from_csv(self):
        ...
        return [
            {"company_code": code, "last_updated": None}
            for code in df["CompanyCode"]
        ]
...
class FilterFactory:
    @staticmethod
    def get_filter(filter_type, *args, **kwargs):
        if filter_type == "Filter1":
            return Filter1(*args, **kwargs)
        elif filter_type == "Filter2":
            return Filter2(*args, **kwargs)
        else:
            raise ValueError(f"Unknown filter type: {filter_type}")

if __name__ == "__main__":
    # Create Filter1 instance using the factory
    filter1 = FilterFactory.get_filter("Filter1", "issuers.csv")
    filter1.execute()

    # Create Filter2 instance using the factory
    filter2 = FilterFactory.get_filter("Filter2", "issuers.csv")
    filter2.execute()
